...
class ChildController extends Controller
{
    ...
    public function index(){
        ...
        $sub_category_list = Category::select('cat_name')->distinct()->latest()->get();
        $sub_category_list_table = Category::select('sub_cat')->distinct()->latest()->limit(10)->get();

        $total_data = Category::select('sub_cat')->distinct()->latest()->count('sub_cat');
...
if (is_float($total_serial_number1)){
    ...
    if (is_float($total_serial_number1)){
        ...
        $total_serial_number = $total_serial_number1;
            ...
            $product_list = Category::select('sub_cat')->distinct()->latest()->limit(10)->get();
            $minus_one = 0;
        }else{
            $total_data = Category::select('sub_cat')->distinct()->latest()->count('sub_cat');
            $total_serial_number1= $total_data/10;
    ...
    if (is_float($total_serial_number1)){
        ...
        $total_serial_number = $total_serial_number1;
            ...
            $product_list = Category::select('sub_cat')->distinct()->latest()->skip($minus_one)->take(10)->get();
        }
