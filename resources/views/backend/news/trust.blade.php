...
<td>{{$cat->created_at}}</td>
      ...
      <td>
            <a class="btn btn-success" type="button" href="{{route('p_news',$cat->id)}}">
             Publish To Bangla Site
        </a>
        
         <a class="btn btn-info" type="button" href="{{route('p_newse',$cat->id)}}">
             Publish To English Site
        </a>
        ...
        <button class="btn btn-danger" type="button" onclick="deleteTag({{ $cat->id }})" data-toggle="tooltip" data-placement="top" title="Delete">
            ...
            <form id="delete-form-{{ $cat->id }}" action="{{ route('admin.news.deletep',$cat->id) }}" method="POST" style="display: none;">
                @method('DELETE')
                @csrf
